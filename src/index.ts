...
type NarrowFilter =
  | { kind: 'fridays' }
  | { kind: 'weekdays' }                   // Mon–Thu starts
  | { kind: 'weekends' }                   // treat as Friday starts
  | { kind: 'weekday'; dow: number }       // specific weekday
  | { kind: 'early' }                      // 1..10
  | { kind: 'mid' }                        // 10..20
  | { kind: 'late' }                       // 21+
  | { kind: 'firsthalf' }                  // 1..15
  | { kind: 'secondhalf' }                 // 16..end
...
function parseNarrowing(msg: string): NarrowFilter | null {
  ...
  // "around the 18th" / "around Fri 18th"
...
function applyNarrowingFilter(dates: Array<{ from: string; price: number }>, f: NarrowFilter) {
  ...
  switch (f.kind) {
    ...
    case 'fridays':
    case 'weekends':
      return byDOW(dow => dow === 5); // Friday
    case 'weekdays':
      return byDOW(dow => dow >= 1 && dow <= 4); // Mon..Thu
    case 'weekday':
...
async function handleChat(req: express.Request, res: express.Response) {
  ...
  // ---- No pending narrowing: run main intents ----
  try {
...
async function runMonthFlow(
  ...
  // Only keep starts that price > 0 (e.g., Bookalet changeover)
  ...
  if (valid.length > 6) {
    // Save context so narrowing terms work next turn
