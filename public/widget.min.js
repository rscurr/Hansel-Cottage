...
function mount(){css();const b=document.createElement("div");b.id="hc-b";b.textContent="💬";document.body.appendChild(b);const w=document.createElement("div");w.id="hc-w";w.innerHTML='<div id="hc-h">Hansel Cottage</div><div id="hc-m"></div><div id="hc-i"><input id="hc-t" placeholder="Ask about availability, pricing, or the PDF..."><button id="hc-s">Send</button></div>';document.body.appendChild(w);const m=w.querySelector("#hc-m"),t=w.querySelector("#hc-t"),s=w.querySelector("#hc-s");function add(u,x){const d=document.createElement("div");d.innerHTML="<strong>"+u+":</strong> "+x;m.appendChild(d);m.scrollTop=m.scrollHeight}s.onclick=async()=>{const text=t.value.trim();if(!text)return;add("You",text);t.value="";try{const out=await chat(text);add("Bot",out.answer||"Sorry, I didn't understand.");if(out.context){try{const pre=document.createElement("pre");pre.textContent=JSON.stringify(out.context,null,2);pre.style.whiteSpace="pre-wrap";pre.style.fontSize="12px";pre.style.background="#f7fafc";pre.style.padding="6px";pre.style.borderRadius="6px";m.appendChild(pre);m.scrollTop=m.scrollHeight;}catch{}}}catch(e){add("Bot","⚠️ Error contacting server.")}};t.addEventListener("keypress",e=>{if(e.key==="Enter")s.click()});b.onclick=()=>{w.style.display=w.style.display==="flex"?"none":"flex";if(w.style.display==="flex")t.focus()};console.log("[hc] widget mounted")}
onReady(mount);
})();
